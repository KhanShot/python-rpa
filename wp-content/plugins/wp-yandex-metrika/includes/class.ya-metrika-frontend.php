...
class YaMetrikaFrontend
{
    ...
    public function registerInlineScripts(){
        ...
        wp_add_inline_script(YAM_SLUG.'_frontend', "(function(){window.wpym.counters = JSON.parse('".json_encode($options['counters'])."');window.wpym.targets = JSON.parse('".json_encode($options['custom_targets'])."');})();");
    }
    ...
    public function printPageCounters()
    {
        ...
        foreach ($options['counters'] as $counter) {
            if (empty($counter['number'])) continue;
            $this->printCounter($counter['number'], $counter['webvisor']);
        }
    }

    public function printCounter($number, $webvisor = true)
    {
        ?>
